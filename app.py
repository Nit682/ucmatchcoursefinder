...
from pywebio.output import clear, put_link, put_text, put_html, put_markdown, put_table
...
def getMatches(db,course,maximumNumberOfResults):
    ...
    for school,url in db.items():
        ...
        if search(course,webtext,101) or search(course_without_ap,webtext,101):
            matches[school] = url
            matchCount += 1
...
if __name__ == '__main__':
    pywebio.start_server(run, port=55)
